...
if(isset($_REQUEST['login']))
{
    ...
    //query wizard
    $data=$con->query("select * from user_register where userid like '$userid' and password like '$password'");
    $row=mysqli_fetch_array($data);

    $comdata=$con->query("select * from company_register where userid like '$userid' and password like '$password'");
    $comrow=mysqli_fetch_array($comdata);

    $didata=$con->query("select * from designer_register where userid like '$userid' and password like '$password'");
    $dirow=mysqli_fetch_array($didata);

    // echo "hii";

    if($row[0]=="" && $comrow[0]=="" && $dirow[0]=="")
    {
       $flag=1;
    //    $data=$con->query("select * from user_register where userid like '$_COOKIE[block]'");
    //    $row=mysqli_fetch_array($data);
    ...
    else
    {
       ...
       if($comrow[0]!="")
       {
            $_SESSION['userid']=$comrow[0];
            $_SESSION['name']=$comrow[2];
            $_SESSION['lastlogintime']=$comrow[7];
            $_SESSION['password']=$comrow[1];
            $_SESSION['location']=$comrow[4];
            $_SESSION['contact']=$comrow[5];
            $_SESSION['since']=$comrow[6];
            // $_SESSION['bdate']=$row[4];
            $_SESSION['logo']=$comrow[3];
            $_SESSION['identity']="dashboard";
       ...
       if($dirow[0]!="")
       {
            $_SESSION['userid']=$dirow[0];
            $_SESSION['name']=$dirow[2];
            $_SESSION['lastlogintime']=$dirow[6];
            $_SESSION['password']=$dirow[1];
            $_SESSION['bdate']=$dirow[4];
            $_SESSION['profile']=$dirow[5];
            $_SESSION['identity']="dashboard";
            $_SESSION['who']="designer";
