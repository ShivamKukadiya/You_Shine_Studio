...
if(isset($_REQUEST['inpackage_offer']))
{
    ...
    if($row[0]=="")
    {
        $in=$con->query("insert into package_offer values '$_REQUEST[name]','$_REQUEST[startdate]','$_REQUEST[enddate]','$_REQUEST[percentage]'");
        $_SESSION['insuccess']=1;
    }
    else
    {
...
{
    $data=$con->query("select * from subcategory where name like '$_REQUEST[categoryid]'");
...
{
    $data=$con->query("select * from package_rules where rules like '$_REQUEST[packageid]'");
...
if(isset($_REQUEST['upadvertisement']))
	{
        ...
        $data=$con->query("select * from advertisement where name like '$_REQUEST[upname]'");
...
if(isset($_REQUEST['company_reg']))
{
    // echo "data success";
    //exit(); */
    $data=$con->query("select * from company_register where userid like '$_REQUEST[userid]'");
    // echo "data success";
    // exit();
    // exit();
    $row = mysqli_fetch_array($data);
    ...
    if(!$row)
    {
        $in=$con->query("insert into company_register values ('$_REQUEST[userid]','$_REQUEST[password]','$_REQUEST[name]','$_REQUEST[profilepic]','$_REQUEST[location]','$_REQUEST[contact]','$_REQUEST[since]','','1')");
    //     echo "success";
    // exit();
...
// if(isset($_REQUEST['company_reg']))
// {
//     $data=$con->query("select * from company_register where name like ('$_REQUEST[userid]','$_REQUEST[password]','$_REQUEST[name]','$_REQUEST[profilepic]','$_REQUEST[location]','$_REQUEST[contact]','$_REQUEST[since]')");

//     $row=mysqli_fetch_array($data);

//     if($row[0]=="")
//     {
//         $up=$con->query("update company_register set name='$_REQUEST[userid]',startdate='$_REQUEST[upstartdate]',enddate='$_REQUEST[upenddate]','percentage=$_REQUEST[uppercentage]' where package_offerid=$_SESSION[upid]");
//         unset($_SESSION['upid']);
...
if(isset($_REQUEST['user_reg']))
{
    // echo "data success";
    //exit(); */
    $data=$con->query("select * from user_register where userid like '$_REQUEST[userid]'");
    // echo "data success";
    // exit();
    // exit();
    $row = mysqli_fetch_array($data);
    ...
    if(!$row)
    {
        $in=$con->query("insert into user_register values ('$_REQUEST[userid]','$_REQUEST[password]','$_REQUEST[name]','$_REQUEST[gender]','$_REQUEST[bdate]','$_REQUEST[profilepic]','','1')");
    //     echo "success";
    // exit();
